...
def fibonacciWithGivenStart(a,b,n):
    if not (isinstance(n,int) or isinstance(n,long)) and (isinstance(a,int) or isinstance(a,long) or isinstance(a,float)) and  (isinstance(b,int) or isinstance(b,long) or isinstance(b,float)):
        raise TypeError
    if n<0:
        return ValueError
    if n==1:
...
def isTwinPrime(y):
    ...
    if y<1:
        raise ValueError
    if isPrime(y) and (isPrime(y+2) or isPrime(y-2)):
        return True
    return False
...
def isCousinPrime(y):
    ...
    if y<1:
        raise ValueError
    if isPrime(y) and (isPrime(y+4) or isPrime(y-4)):
        return True
    return False    
...
def isSexyPrime(y):
    ...
    if y<1:
        raise ValueError
    if isPrime(y) and (isPrime(y+6) or isPrime(y-6)):
        return True
    return False
...
def isnDistancePrime(n,p):
    ...
    if p<1:
        raise ValueError
    if isPrime(p) and (isPrime(p+n) or isPrime(p-n)):
        return True
    return False  
...
def polygonal(sides,n):
    if not ((isinstance(n,int) or isinstance(n,long)) and (isinstance(sides,int) or isinstance(sides,long))):
        raise TypeError
    if n<0 or sides <1:
        raise ValueError
...
def shuffle1(List):
    ...
    outputList=list(range(len(List)))
...
def radixSort(List):
    ...
    #Finds the longest list in the list (equivalently, the number with the most digits).
    for i in output:
        while len(i)<longest:
            i.append(0)
            #Makes all list in the lists of equal length, without changing the value they represent.
...
#The output is a list, first containing sorted small
#then sorted big and finally
...
def quickSort(List):
    ...
    return sortedSmall+[pivot]+sortedBig      
...
class matrix:
    #determines if the given input is a valid matrix of intergers,longs or floats
    #by checking for a list of lists of the same length and checking if all the
    #entries in the sublists are intergers,longs or floats
    ...
    def next(self):
        self.index+=1
        if self.index>self.dimension[0]:
            raise StopIteration
        return self.listOfRows[self.index-1]

    def __getitem__(self,integer):
        if not isinstance(integer,int):
            raise TypeError
        return self.listOfRows[integer]
    ...
    def __add__(self,other):
        ...
        if not m==n:
            raise TypeError
        ans=self.copy().listOfRows
        for x in range(m[0]):
            for y in range(m[1]):
                ans[x][y]=self[x][y]+other[x][y]
        return matrix(*ans)
    
    def __mul__(self,other):
        m1=self.dimension[0]
        m2=self.dimension[1]
        n1=other.dimension[0]
        n2=other.dimension[1]
        if not m2==n1:
            raise TypeError
        ...
        for x in range(m1):
         ...
         #loops through colums of M and rows of N adding products
                tempAnswer.append(sum(map(lambda z:self[x][z]*other[z][y],list(range(m1)))))
        return matrix(*map(lambda x:tempAnswer[x*m1:(x+1)*m1],list(range(n2))))
...
def matrixMultiply(M,N):
     ...
     #Finds the row and column 'sizes', 1 represents column number; 2 row number
     m1=len(M)
     m2=len(M[0])
     n1=len(N)
     n2=len(N[0])
     ...
     #loops through colums of M and rows of N adding products
             tempAnswer.append(sum(map(lambda z:M[x][z]*N[z][y],list(range(m1)))))
     return map(lambda x:tempAnswer[x*m1:(x+1)*m1],list(range(n2)))
...
def vectorDotProduct(V,U):
    ...
    if not len(V)==len(U):
        raise TypeError
    return sum(map(lambda x:V[x]*U[x],list(range(len(U)))))
...
def vectorCrossProduct(V,U):
    ...
    if not len(V)==len(U)==3:
        raise TypeError
    return [V[1]*U[2]-U[1]*V[2],U[0]*V[2]-V[0]*U[2],V[0]*U[1]-U[0]*V[1]]
...
def degd(graph,d):
    if not (isinstance(graph,networkx.classes.graph.Graph) and isinstance(d,int)):
        raise TypeError
    if d<0:
        raise ValueError
    output=[]
...
def isPermutation(List):
    if not isinstance(List,list):
        return False
    n=len(List)
    if not set([x+1 for x in range(n)])==set(List):
        return False
    return True
...
def disjointCycle(permutation):
    if not isPermutation(permutation):
        raise TypeError
    identityPermutation=map(lambda x:x+1,list(range(len(permutation))))
...
def isEven(permutation):
    ...
    n=len(permutation)
    if permutation==[x+1 for x in range(n)]:
        return True
...
def symmetricGroupList(n):
    ...
    if n<1:
        raise ValueError
    return permutations([x+1 for x in range(n)])
...
def isEuclideanTriangle(side1,side2,side3):
    for side in [side1,side2,side3]:
        if not (isinstance(side,int) or isinstance(side,long) or isinstance(side,float)):
            raise TypeError
...
def isHyperbolicTriangle(angle1,angle2,angle3):
    ...
    for angle in angles:
        if not ((isinstance(angle1,int) or isinstance(angle1,long) or isinstance(angle1,float))):
            raise TypeError
    if sum(angles)<math.pi:
        return True
    return False
...
def guessLimit(sequence,epsilon,breakLimit=300000000000000):
    ...
    while True:
       ...
       if abs(sequence(n)-sequence(n+1))<epsilon: 
           if abs(sequence(n)-sequence(n+1))<epsilon and abs(sequence(n)-sequence(n+1))<epsilon and abs(sequence(n)-sequence(n+2))<epsilon and abs(sequence(n+1)-sequence(n+3))<epsilon and abs(sequence(n)-sequence(n+4))<epsilon:
               return sequence(n)
       if n>breakLimit:
           return     
